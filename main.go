...
import (
	...
	"github.com/lonegunmanb/terraform-provider-azuread-index/pkg"
)
...
Required flags:
  -scan-path string
        Path to scan for Terraform provider services (e.g., ./tmp/terraform-provider-azuread/internal/services)
  -package-path string
        Base package path for the provider (e.g., github.com/hashicorp/terraform-provider-azuread)
  -version string
        Version of the provider (e.g., v2.53.1)
...
Example:
  %s -scan-path ./tmp/terraform-provider-azuread/internal/services \
    -package-path github.com/hashicorp/terraform-provider-azuread \
    -version v2.53.1 \
    -output ./output/index
`, os.Args[0], os.Args[0])
	...
	fmt.Printf("  📋 Main index: %s/terraform-provider-azuread-index.json\n", *outputDir)
